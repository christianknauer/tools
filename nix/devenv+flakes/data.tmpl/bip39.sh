...
escape-output-if-needed() if test -t 1; then
	\cat -v
else
	\cat
fi
...
check-mnemonic() if [[ $# =~ ^(12|15|18|21|24)$ ]]; then
	...
	for word; do
		if ((${wordlist_reverse[$word]})); then
			dc_script+=" 2048*${wordlist_reverse[$word]} 1-+"
		else
			return 1
		fi
	done
...
function mnemonic-to-seed() {
	...
	if getopts hpP o; then
		...
		P)
			...
			if [[ $REPLY == "$passphrase" ]]; then
				BIP39_PASSPHRASE=$passphrase $FUNCNAME "$@"
			else
				echo "passphrase input error" >&2
				return 3
			fi
			;;
		esac
...
then
...
elif ((${#wordlist[@]} != 2048)); then
	echo 1>&2 "unexpected number of words (${#wordlist[@]}) in wordlist array"
	return 2
elif [[ $1 =~ ^(128|160|192|224|256)$ ]]; then
	$FUNCNAME $(openssl rand -hex $(($1 / 8)))
elif [[ $1 =~ ^([[:xdigit:]]{2}){16,32}$ ]]; then
	local hexnoise="${1^^}"
	local -i ENT=${#hexnoise}*4 #bits
	if ((ENT % 32)); then
		echo 1>&2 entropy must be a multiple of 32, yet it is $ENT
		return 2
	fi
	{
	...
	echo 1>&2 parameters have insufficient entropy or wrong format
	return 4
fi
