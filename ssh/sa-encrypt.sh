...
DebugMsg 1 "using exec files \"${ENCRYPT}\", \"${DECRYPT}\""
...
if [ ! "${PUBKEYFILE}" == "" ]; then
	...
	read KeyType RestOfLine < ${PUBKEYFILE} 
	    ...
	    InfoMsg "using public key ($KeyIDShort) from \"$PUBKEYFILE\""
	    DESTKEY=$PublicKey
        else 
...
case $ec in
    0) DebugMsg 1 "agent provides ${NROFKEYS} key(s)"; cat ${INFILE} | ${ENCRYPT} > ${ENCFILE};;
...
while IFS='' read -r LinefromFile || [[ -n "${LinefromFile}" ]]; do
    ...
    if [[ $KeyType = ssh-rsa ]]; then
            if [[ ${DESTKEY} == "unspecified" || ${PublicKey} = ${DESTKEY} ]]; then
                ((RSACounter++))
		EncOutputFile="${OUTFILE}.${KeyIDShort}"
		InfoMsg "key #$Counter ($KeyIDShort) is accepted (result written to \"$EncOutputFile\")" #: ${LinefromFile}"
	        ...
	        DebugMsg 1 "key #$Counter ($KeyIDShort) is rejected (not the destination key)" #: ${LinefromFile}"
	    fi
    else 
